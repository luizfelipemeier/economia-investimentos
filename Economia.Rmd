...
![Fonte: [Banco Central do Brasil](https://www.bcb.gov.br/controleinflacao/metainflacao)](Images/inflacao.png){.img-responsive .center}
...
if(nrow(df)>1){
        ...
        # Transformando coluna ano em data
        df$Ano <- as.Date(paste0(df$Ano, "-01-01"))
        ...
        # Numero problematico
        df$Meta[df$Ano == as.Date("2020-01-01")] <- "4.0"
        
        # Transformar Ano em formato de texto para evitar problemas de exibição
        df$Ano <- as.character(df$Ano)

        
        # Criar o gráfico com highcharter
        plot_ly(df, x = ~Ano, y = ~Meta, name = "Meta", type = "scatter", mode = "lines") %>%
        add_trace(x = ~Ano, y = ~Intervalo_inferior, name = "Intervalo_inferior", type = "scatter", mode = "lines", line = list(dash = "dash")) %>%
        add_trace(x = ~Ano, y = ~Intervalo_superior, name = "Intervalo_superior", type = "scatter", mode = "lines", line = list(dash = "dash")) %>%
        layout(
          title = "Metas da inflação - CMN",
          yaxis = list(title = "Variação %"),
          xaxis = list(title = "Ano"),
          legend = list(x = 0.5, y = -0.2, orientation = "h")  # Posição e orientação da legenda
        )
