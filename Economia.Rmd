...
```{r include=FALSE}
#instalando pacotes na pasta do repositorio
install.packages(c("rmarkdown", 
                   ...
                   "GetBCBData",
                   "curl",
                   "rmarkdown"), lib = local_lib, repos = "https://cloud.r-project.org")
...
O produto interno bruto pode ser mensurado tanto na ótica da demanda, quanto na ótica da oferta.
...
PMC = sidra('/t/8880/n1/all/v/7169/p/all/c11046/56733/d/v7169%205', format(Sys.Date() - months(26), "%Y-%m")) %>% 
  ...
  mutate(Cor = ifelse(PMC_MENSAL > 0, "azul", "vermelho")) %>% 
  filter(Data >= format(Sys.Date() - months(25), "%Y-%m"))

ggplot(PMC, aes(x = Data, y = PMC_MENSAL, fill = Cor)) +
  geom_col()+
  labs(x = "", y = "Variação %") +
  scale_fill_manual(
    values = c(azul = "#82C35F",
    vermelho = "#E75A5A")
  ) +
  theme_light() +
...
PMS = sidra('/t/5906/n1/all/v/7167/p/all/c11046/56725/d/v7167%205', format(Sys.Date() - months(26), "%Y-%m")) %>% 
  ...
  mutate(Cor = ifelse(PMS_MENSAL > 0, "azul", "vermelho"))%>% 
  filter(Data >= format(Sys.Date() - months(25), "%Y-%m"))

ggplot(PMS, aes(x = Data, y = PMS_MENSAL, fill = Cor)) +
  geom_col()+
  labs(x = "", y = "") +
  scale_fill_manual(
    values = c(azul = "#82C35F",
